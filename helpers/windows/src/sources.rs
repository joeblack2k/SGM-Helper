...
fn known_scan_candidates(
    ...
    if matches!(default_kind, SourceKind::Windows) {
        candidates.push(CandidatePath {
            id: "retroarch_program_files".to_string(),
            label: "RetroArch Program Files".to_string(),
            kind: SourceKind::RetroArch,
            profile: EmulatorProfile::RetroArch,
            save_path: PathBuf::from("C:\\\\RetroArch\\\\saves"),
            rom_path: PathBuf::from("C:\\\\RetroArch\\\\roms"),
            recursive: true,
            origin: "autoscan-known".to_string(),
        });
        candidates.push(CandidatePath {
            id: "emudeck_windows".to_string(),
            label: "EmuDeck Windows".to_string(),
            kind: SourceKind::Windows,
            profile: EmulatorProfile::RetroArch,
            save_path: PathBuf::from("C:\\\\Emulation\\\\saves"),
            rom_path: PathBuf::from("C:\\\\Emulation\\\\roms"),
            recursive: true,
            origin: "autoscan-known".to_string(),
        });
        if let Ok(profile) = env::var("USERPROFILE") {
            ...
            candidates.push(CandidatePath {
                id: "retroarch_userprofile".to_string(),
                label: "RetroArch UserProfile".to_string(),
                kind: SourceKind::RetroArch,
                profile: EmulatorProfile::RetroArch,
                save_path: profile.join("AppData/Roaming/RetroArch/saves"),
                rom_path: profile.join("Emulation/roms"),
                recursive: true,
                origin: "autoscan-known".to_string(),
            });
            candidates.push(CandidatePath {
                id: "snes9x_userprofile".to_string(),
                label: "Snes9x UserProfile".to_string(),
                kind: SourceKind::Custom,
                profile: EmulatorProfile::Snes9x,
                save_path: profile.join("snes9x/save"),
                rom_path: profile.join("roms/snes"),
                recursive: true,
                origin: "autoscan-known".to_string(),
            });
        }
...
mod tests {
    ...
    fn default_windows_source_uses_save_layout() {
        ...
        let source = default_source(&cfg, SourceKind::Windows).unwrap();
        assert!(source.save_roots[0].to_string_lossy().ends_with("/saves"));
    }
