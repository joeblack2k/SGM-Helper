...
fn dispatch(cli: Cli, loaded: LoadedConfig) -> Result<()> {
    match cli.command {
        ...
        Commands::Sync {
            ...
            let auth =
                ensure_auth_or_auto_enroll(&cfg, cli.verbose, cli.quiet, SourceKind::Windows)?;
        ...
        Commands::Service { command } => {
            let service_name = "SGM Windows Helper Service";
            let binary_path =
